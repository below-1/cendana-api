...
export async function snapshotReport(options: ReportOptions) {
  ...
  const [ { total: totalSale } ] = await prisma.$queryRawUnsafe<TotalResult>(`
  ...
  const [ { total: piutang } ] = await prisma.$queryRawUnsafe<TotalResult>(`
  ...
  const [ { total: pembelianBarangDagang } ] = await prisma.$queryRawUnsafe<TotalResult>(`
    select coalesce(sum(o."grandTotal"), 0) as total from "Order" o 
      where o."orderType" = 'BUY'
      and o."createdAt" between '${t0}' and '${t1}'`)

  const [ { total: totalOpex } ] = await prisma.$queryRawUnsafe<TotalResult>(`
    select coalesce(sum(t.nominal), 0) as total from "Transaction" t 
      where t."createdAt" >= '${t0}' and t."createdAt" <= '${t1}' and t."opexId" > 0`)

  const [ { total: modalAwal } ] = await prisma.$queryRawUnsafe<TotalResult>(` 
    select re.nominal as total from "RecordEquity" as re
      where re."createdAt" <= '${t0}'
      limit 1`)

  const [ { total: utangDagang } ] = await prisma.$queryRawUnsafe<TotalResult>(`
    select coalesce(sum(o."grandTotal"), 0) as total from "Order" o 
      where o."orderType" = 'BUY'
      and o."createdAt" between '${t0}' and '${t1}'`)

  const [ { total: prive } ] = await prisma.$queryRawUnsafe<TotalResult>(` 
  ...
  const [ { total: penambahanModalUsaha } ] = await prisma.$queryRawUnsafe<TotalResult>(` 
    select coalesce(sum(t.nominal), 0) as total from "EquityChange" as ec
      left join "Transaction" t on t."equityChangeId" = ec.id
      where 
        t."createdAt" >= '${t0}' 
  ...
  const [ { total: roc } ] = await prisma.$queryRawUnsafe<TotalResult>(` 
  ...
  const [ { total: investment } ] = await prisma.$queryRawUnsafe<TotalResult>(`   
    select coalesce(sum(t.nominal), 0) as total from "Transaction" t 
  ...
  const report = await prisma.financeReport.create({
    data: {
      ...
      totalBiayaPengeluaran: totalOpex_dec.add(peralatan_dec),
      pajak: pajak_dec,
